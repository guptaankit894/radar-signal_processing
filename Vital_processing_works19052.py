#Read raw radar data → FFT to identify distances 
#→ Generate map to identify distinct people → 
#Separate signals using beamforming → Extract breathing and heart signals 
#→ Calculate HR & RR import os
...
# -- 1. Decode raw CSV → ADC cube (64 chirps) --
#def decode_iwr6843_csv(filename,
                       #num_rx=4,
                       #num_adc_samples=256,
                       #num_chirps=128):
    # load as complex
    #df = pd.read_csv(filename, header=None)
    #df_complex = df.map(lambda x: complex(x.replace(' ', '')))
    #arr = df.values  # shape: (num_rows, num_rx*num_adc_samples)
    #num_rows, num_cols = arr.shape
    #assert num_cols == num_rx * num_adc_samples, (
       #f"Expected {num_rx*num_adc_samples} columns, got {num_cols}"
    #)
    ## number of frames
    #num_frames = num_rows // num_chirps
    #assert num_frames * num_chirps == num_rows, "Rows must be multiple of num_chirps"
    # reshape: (frames, chirps, rx, samples)
    #tmp = arr.reshape(num_frames, num_chirps, num_rx, num_adc_samples)
    # transpose to (frames, rx, chirps, samples)
    #adc = tmp.transpose(0, 2, 1, 3)
    #return adc
...
FILE = "223_TopFront_4m_0deg_Man3_CondF_CondM_CondC_ADC256_Chirp64_FR60_RE60_Gain30_FS30_IWR_0.bin"
adc = decode_iwr6843_data(FILE)
rp  = compute_range_profiles(adc)
ra_map = compute_range_angle_map(rp, n_angle_bins=64)
targets = detect_targets_physical(ra_map, 3, min_dist_m=0.5, B=3.9e9, n_angle_bins=256, R_mean=4.0) #(variate n_angle_bins according to the adc samples)
